...
use super::{gds_path, LIB_DIR};

fn schematic_layout_path(_ctx: &SubstrateCtx, name: &str, view: View) -> Option<PathBuf> {
    let lib_path = PathBuf::from(LIB_DIR);
    match view {
        View::Schematic => Some(
            lib_path
                .join(name)
                .join(format!("sky130_fd_bd_sram__{name}.spice")),
        ),
        View::Layout => Some(gds_path(name)),
        _ => None,
    }
}
